...
A note has `id`, `title`, `content`, `created_at`, and `updated_at`. Title is required, non-blank, max 200 characters. Content is required, non-blank, max 10,000 characters. Invalid input is rejected with `422 Unprocessable Entity` and no data change. Errors use a consistent JSON shape (`error`, optional `details`). Run and test commands are in `README.md`.
